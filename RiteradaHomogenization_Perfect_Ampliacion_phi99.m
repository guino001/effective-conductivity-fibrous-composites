...
for jj=1:length(phi)
 ...
 %% AHM n=0;
lamInt=ahm2f_Paralelogramo_Cortas_2023_No0(cm,cf,Vf,V,S1,H1,H2); % VanFo Fi
% CENo(jj,:,tt)=ahm2f_Paralelogramo_Cortas_2023_No0(lamInt(1),cf,Vc,V,S1,H1,H2); % VanFo Fi
%% AHM n=1;
% lamInt =ahm2f_Paralelogramo_Cortas_2023_No1(cm,cf,Vf,V,S1,H1,H2);
% CENo(jj,:,tt) =ahm2f_Paralelogramo_Cortas_2023_No1(lamInt(1),cf,Vc,V,S1,H1,H2);

%% AHM n=2;
% lamInt =ahm2f_Paralelogramo_Cortas_2023_No2(cm,cf,Vf,V,S1,H1,H2);
% CENo(jj,:,tt) =ahm2f_Paralelogramo_Cortas_2023_No2(lamInt(1),cf,Vc,V,S1,H1,H2);

%  CENo3(j,:,tt) =ahm2f_Paralelogramo_Cortas_2023_No3(cm,cf,V2,V,S1); 
%  CENo4(j,:,tt) =ahm2f_Paralelogramo_Cortas_2023_No4(cm,cf,V2,V,S1); 

%  lamInt =ahm2f_Paralelogramo_Cortas_2023_No5(cm,cf,Vf,V,S1,H1,H2);
%  CENo(jj,:,tt) =ahm2f_Paralelogramo_Cortas_2023_No5(lamInt(1),cf,Vc,V,S1,H1,H2);

% lamInt =conductividad_efectiva_orden_m(cm,cf,Vf,V,S1,H1,H2,30);
 CENo(jj,:,tt) =conductividad_efectiva_orden_m(lamInt(1),cf,Vc,V,S1,H1,H2,30);
end                                 
end
...
colores = [[0,0,1];[.6,.6,0];[0,1,0];[1,0,0];[0,1,1];...
    [1,0,1];[.8,0.5,.8];[0.5,0.5,0.5];[0.5,0.5,1];[0.8,0.8,0];[0.1,0.8,0];[.8,0,0];[0,.8,.8];[.6,.3,0];[0,1,0];[4,0,0]];
marca = ['ko-'; 'ko-';'ko-';'ko-'; 'ko-';'ko-';'ko-';'ks-';'ko-';'ko-';'ks-'];
letras = ['a)';'b)';'c)';'d)';'e)';'f)'];
...
% Primer plot - HU
h1 = plot(phi, HU, 'k-', 'LineWidth', 2, 'MarkerEdgeColor', 'k', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h1];
% Plot MIT
h2 = plot(phi, MIT, 'b-.', 'LineWidth', 2, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h2];
% Plots para diferentes valores de alpha

h3 = plot(phi, BU, 'r-', 'LineWidth', 2.5, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h3];
for tt = 1:length(alpha)
    h_temp = plot(phi, CENo(:,1,tt),'LineWidth', 2, 'Color', colores(tt,:));
    h = [h, h_temp];
end
h3 = plot(phi, BU, 'r-', 'LineWidth', 2.5, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
% Plot HL
h4 = plot(phi, HL, 'k-.', 'LineWidth', 2, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h4];

h5 = plot(phi, BL, 'r-.', 'LineWidth', 2.5, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h5];

hold off
text(phi(3),HU(3),['$\rho = $', num2str(xas,3)],'fontsize', 14, 'interpreter', 'latex')
% Crear las etiquetas para la leyenda
legend_labels = {'Hashin Shtrikman upper bound'};
legend_labels{end+1} = '$\sigma_{100}$ sqr(in MITYUSHEV )';
...
grid minor
box on
% CREAR AMPLIACIÓN (ZOOM)PRIMERA
% PARÁMETROS AJUSTABLES PARA LA AMPLIACIÓN
x_min_zoom = 0.;      % Límite inferior en x
x_max_zoom = .5;    % Límite superior en x
y_min_zoom = 0.8;      % Límite inferior en y - ¡AJUSTA ESTE VALOR!
y_max_zoom = 4;      % Límite superior en y - ¡AJUSTA ESTE VALOR!

% CREAR AMPLIACIÓN (ZOOM)
axes('Position', [0.6 0.3 0.25 0.25]) % [left bottom width height]
box on
hold on

% Replotear las mismas curvas pero con los límites del zoom
plot(phi, HU, 'k-', 'LineWidth', 2)
plot(phi, MIT, 'b-.', 'LineWidth', 2)
plot(phi, BU, 'r-', 'LineWidth', 2)
for tt = 1:length(alpha)
    plot(phi, CENo(:,1,tt), 'LineWidth', 1.2, 'Color', colores(tt,:))
end
plot(phi, BU, 'r-', 'LineWidth', 2)
plot(phi, HL, 'k-.', 'LineWidth', 2)
plot(phi, BL, 'r--', 'LineWidth', 2)
% Configurar los límites del zoom
xlim([x_min_zoom, x_max_zoom])
ylim([y_min_zoom, y_max_zoom])  % Altura controlada manualmente

% Configuraciones adicionales del recuadro de zoom
grid on
xlabel('$\kappa_2/\kappa_1$', 'interpreter', 'latex', 'FontSize', 10)
ylabel('$\hat{\kappa}/\kappa_1$', 'interpreter', 'latex', 'FontSize', 10)
title('Zoom zone', 'FontSize', 10)
a=round((x_max_zoom-x_min_zoom)/5);
%xticks([x_min_zoom:a: x_max_zoom])
xticks([0,.1:.1:.5])
% Dibujar rectángulo en el gráfico principal
main_ax = gca;
axes(main_ax) % Volver al eje principal
rectangle('Position', [x_min_zoom, y_min_zoom, x_max_zoom-x_min_zoom, y_max_zoom-y_min_zoom], ...
          'EdgeColor', 'r', 'LineStyle', '--', 'LineWidth', 1.5)

% Configuraciones adicionales del recuadro de zoom
grid on
xlabel('Fiber volume fraction', 'interpreter', 'latex', 'FontSize', 10)
ylabel('$\hat{\kappa}/\kappa_1$', 'interpreter', 'latex', 'FontSize', 10)
%title('Ampliación', 'FontSize', 10)

% Opcional: Dibujar un rectángulo en el gráfico principal que muestre el área ampliada
main_ax = gca;
axes(main_ax) % Volver al eje principal
%rectangle('Position', [0.6, y_min-0.05*y_range, 0.184, y_range*1.1], ...
 %         'EdgeColor', 'r', 'LineStyle', '--', 'LineWidth', 1.5)
 %%%%%%%% Segunda ampliacion
 % CREAR AMPLIACIÓN (ZOOM)
% PARÁMETROS AJUSTABLES PARA LA AMPLIACIÓN
x_min_zoom = 0.3;      % Límite inferior en x
x_max_zoom = .5;    % Límite superior en x
y_min_zoom = 1.5;      % Límite inferior en y - ¡AJUSTA ESTE VALOR!
y_max_zoom = 3.5;      % Límite superior en y - ¡AJUSTA ESTE VALOR!

% CREAR AMPLIACIÓN (ZOOM)
axes('Position', [0.2 0.3 0.25 0.25]) % [left bottom width height]
box on
hold on
...
% CALCULATE_SIGMA_RATIO Calcula sigma_100(f,rho)/sigma_2 segÃºn la fÃ³rmula proporcionada
%
% Inputs:
%   f   - parÃ¡metro libre f (puede ser escalar o vector)
%   rho - parÃ¡metro libre rho (puede ser escalar o vector)
%
% Output:
