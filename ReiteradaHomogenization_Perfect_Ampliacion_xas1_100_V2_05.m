...
%  CENo(jj,:,tt) =conductividad_efectiva_orden_m(lamInt(1),cf(jj),Vc,V,S1,H1,H2,30);
end                                 
end
...
h3 = plot(xas, BU, 'r-', 'LineWidth', 2.5, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
% Plot HL
h4 = plot(xas, HL, 'k-.', 'LineWidth', 2, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h4];

h5 = plot(xas, BL, 'r-.', 'LineWidth', 2.5, 'MarkerEdgeColor', 'r', ...
          'MarkerFaceColor', 'w', 'MarkerSize', 6);
h = [h, h5];
...
x_min_zoom = 1;      % Límite inferior en x
x_max_zoom = 10;    % Límite superior en x
y_min_zoom = 1;      % Límite inferior en y - ¡AJUSTA ESTE VALOR!
y_max_zoom = 4.2;      % Límite superior en y - ¡AJUSTA ESTE VALOR!
...
plot(xas, HU, 'k-', 'LineWidth', 2)
plot(xas, MIT, 'b-.', 'LineWidth', 2)
plot(xas, BU, 'r-', 'LineWidth', 2)
for tt = 1:length(alpha)
    plot(xas, CENo(:,1,tt), 'LineWidth', 1.2, 'Color', colores(tt,:))
end
plot(xas, BU, 'r-', 'LineWidth', 2)
plot(xas, HL, 'k-.', 'LineWidth', 2)
plot(xas, BL, 'r--', 'LineWidth', 2)
...
% CREAR AMPLIACIÓN (ZOOM)
% PARÁMETROS AJUSTABLES PARA LA AMPLIACIÓN
x_min_zoom = 7;      % Límite inferior en x
x_max_zoom = 10;    % Límite superior en x
y_min_zoom = 2;      % Límite inferior en y - ¡AJUSTA ESTE VALOR!
y_max_zoom = 3;      % Límite superior en y - ¡AJUSTA ESTE VALOR!

% CREAR AMPLIACIÓN (ZOOM)2
...
function sigma_100 = point7_upper_bound(sigma1, sigma2)
    ...
    % Términos DEL NUMERADOR (4 Términos)
    ...
    % Términos DEL DENOMINADOR (5 Términos)  
    term_d1 = sigma1.^4;  
